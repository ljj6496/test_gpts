...
import quotations_volatility_trend_daily
...
def main():
    ...
    Example:
        >>> df = get_quotations_volatility_trend_daily(fid_cond_mrkt_div_code="W", fid_input_iscd="58J297")
    """
    try:
        # pandas 출력 옵션 설정
        ...
        # ELW 변동성 추이(일별) 파라미터 설정
        logger.info("API 파라미터 설정 중...")
        fid_cond_mrkt_div_code = "W"  # 조건시장분류코드
        fid_input_iscd = "58J297"  # 입력종목코드
        
        # API 호출
        logger.info("API 호출 시작: ELW 변동성 추이(일별)")
        result = quotations_volatility_trend_daily.get_quotations_volatility_trend_daily(
