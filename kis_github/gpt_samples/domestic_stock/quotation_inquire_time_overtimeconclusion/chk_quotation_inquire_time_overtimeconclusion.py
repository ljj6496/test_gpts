...
from quotation_inquire_time_overtimeconclusion import get_quotation_inquire_time_overtimeconclusion
...
def main():
    ...
    try:
        result1, result2 = get_quotation_inquire_time_overtimeconclusion(env_dv="real", fid_cond_mrkt_div_code="J", fid_input_iscd="005930", fid_hour_cls_code="1")
    ...
    numeric_columns = ['시간외 단일가 현재가', '시간외 단일가 전일 대비', '시간외 단일가 전일 대비율', 
                      ...
                      '시간외 단일가 예상 체결 대비', '시간외 단일가 예상 체결 대비율', '시간외 단일가 예상 거래량']
    ...
    # 숫자형 컬럼 소수점 둘째자리까지 표시
    numeric_columns = ['주식 현재가', '전일 대비', '전일 대비율', '매도호가', '매수호가', '누적 거래량', '체결 거래량']
