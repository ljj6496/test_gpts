...
import quotations_inquire_index_timeprice
...
def main():
    ...
    Returns:
        - DataFrame: 국내업종 시간별지수(분) 결과
    
    Example:
        >>> df = get_quotations_inquire_index_timeprice(fid_input_hour_1="60", fid_input_iscd="0001", fid_cond_mrkt_div_code="U")
    """
    try:
        # pandas 출력 옵션 설정
        ...
        # 국내업종 시간별지수(분) 파라미터 설정
        logger.info("API 파라미터 설정 중...")
        fid_input_hour_1 = "60"  # ?입력 시간1
        fid_input_iscd = "0001"  # 입력 종목코드
        fid_cond_mrkt_div_code = "U"  # 조건 시장 분류 코드
        
        # API 호출
        logger.info("API 호출 시작: 국내업종 시간별지수(분)")
        result = quotations_inquire_index_timeprice.get_quotations_inquire_index_timeprice(
