...
from trading_inquire_account_balance import get_trading_inquire_account_balance
...
def main():
    ...
    try:
        result1, result2 = get_trading_inquire_account_balance(cano="81180744", acnt_prdt_cd="21")
    ...
    numeric_columns = ['매입금액', '평가금액', '평가손익금액', '신용대출금액', '실제순자산금액', '전체비중율']
    ...
    # 숫자형 컬럼 소수점 둘째자리까지 표시
    numeric_columns = ['매입금액합계', '순자산총금액', '대출금액합계', '평가손익금액합계', '평가금액합계', '총자산금액', '총대출금액총융자대출금액', 'CMA자동대출금액', '총담보대출금액', '대주평가금액', '신용융자금액', 'OCL_APL대출금액', '질권설정금액', '외화평가총액', '총예수금액', 'CMA평가금액', '예수금액', '총대용금액', '당일미수금액', '해외주식평가금액1', '해외채권평가금액', 'MMFCMA담보대출금액', '청약예수금액', '공모주청약자금대출사용금액', '기업신용공여대출금액']
