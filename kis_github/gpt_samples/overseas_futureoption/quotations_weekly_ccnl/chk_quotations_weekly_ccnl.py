...
def main():
    ...
    Example:
        >>> df1, df2 = get_quotations_weekly_ccnl(srs_cd="6AM24", exch_cd="CME", start_date_time="", close_date_time="20240402", qry_tp="Q", qry_cnt="30", qry_gap="", index_key="")
    """
    try:
        # pandas 출력 옵션 설정
        ...
        # 해외선물 체결추이(주간) 파라미터 설정
        logger.info("API 파라미터 설정 중...")
        srs_cd = "DXM24"  # 종목코드
        exch_cd = "ICE"  # 거래소코드
        start_date_time = ""  # 조회시작일시
        close_date_time = "20250630"  # 조회종료일시
        qry_tp = ""  # 조회구분
        qry_cnt = "30"  # 요청개수
        qry_gap = ""  # 묶음개수
        index_key = ""  # 이전조회KEY
        ...
        # API 호출
        logger.info("API 호출 시작: 해외선물 체결추이(주간)")
        result1, result2 = quotations_weekly_ccnl.get_quotations_weekly_ccnl(
            srs_cd=srs_cd,  # 종목코드
