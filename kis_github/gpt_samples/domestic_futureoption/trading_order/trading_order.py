...
def post_trading_order(
    ...
    * 선물옵션 운영시간 외 API 호출 시 애러가 발생하오니 운영시간을 확인해주세요.
    ...
    Args:
        ...
        shtn_pdno (str): [필수] 단축상품번호 (ex. 종목번호, 선물 6자리 (예: 101S03), 옵션 9자리 (예: 201S03370))
        ord_qty (str): [필수] 주문수량
    ...
    Example:
        >>> df = post_trading_order(env_dv="real", ord_dv="day", ord_prcs_dvsn_cd="02", cano="81180744", acnt_prdt_cd="03", sll_buy_dvsn_cd="02", shtn_pdno="167R12", ord_qty="1", unit_price="0", nmpr_type_cd="02", krx_nmpr_cndt_cd="0", ord_dvsn_cd="02")
        >>> print(df)
    """
    ...
    if shtn_pdno == "":
        raise ValueError("shtn_pdno is required (e.g. '101S03', '201S03370')")
    ...
    res = kis._url_fetch(API_URL, tr_id, "", params, postFlag=True)
