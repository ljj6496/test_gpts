...
def get_quotations_inquire_asking_price(
    auth: str,  # 사용자권한정보
    excd: str,  # 거래소코드
    symb: str,  # 종목코드
    ...
    Example:
        >>> df1, df2, df3 = get_quotations_inquire_asking_price(auth="your_auth_token", excd="NAS", symb="TSLA")
        >>> print(df1)
        >>> print(df2)
        >>> print(df3)
    """
    ...
    if res.isOK():
        ...
        if tr_cont in ["M", "F"]:
            ...
            time.sleep(0.1)
            return get_quotations_inquire_asking_price(
                auth,
                excd,
                symb,
